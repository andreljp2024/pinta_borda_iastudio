...
export const PdvView: React.FC<{ onOpenShiftModal: () => void }> = ({ onOpenShiftModal }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="bg-[#ffffff] rounded-2xl p-4 border border-[#fbcfe8] shadow-2xs mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#fff0f5] text-[#db2777] flex items-center justify-center font-bold border border-[#fbcfe8]">
            <Store className="w-5 h-5" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-[#9b4f76] font-mono-craft">
                Ponto de Venda • Rio Anil Shopping
              </span>
              <span className="inline-block w-2 h-2 rounded-full bg-[#3c6b54] animate-pulse" />
            </div>
            <div className="text-sm font-bold text-[#380c25]">
              Operador em Atendimento:{' '}
              <span className="text-[#f43f7e] font-semibold">
        ...
        <div className="flex items-center gap-2 font-mono-craft">
          {activeShift ? (
            ...
            >
              <Clock className="w-3.5 h-3.5 text-[#f43f7e]" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="bg-[#ffffff] rounded-2xl p-4 border border-[#fbcfe8] shadow-2xs space-y-3">
            {/* Search */}
            <div className="relative">
              <Search className="w-4 h-4 text-[#9b4f76] absolute left-3 top-3" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Pesquisar por nome, SKU (ex: TUT-001) ou ateliê..."
                className="w-full pl-9 pr-3 py-2 text-sm rounded-xl border border-[#fbcfe8] focus:outline-none focus:ring-2 focus:ring-[#f43f7e]/20 focus:border-[#f43f7e] bg-white placeholder-[#9b4f76]"
              />
            </div>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-full shrink-0 font-medium transition-colors cursor-pointer ${
                  selectedCategoryId === 'all'
                    ? 'bg-[#380c25] text-[#ffffff] font-semibold'
                    : 'bg-[#fff0f5] text-[#863b63] hover:bg-[#fce7f3]'
                }`}
              >
                Todos
              </button>
              ...
              return (
                <div
                  ...
                  className={`bg-[#ffffff] rounded-xl p-3 border transition-all flex flex-col justify-between select-none ${
                    isOutOfStock
                      ? 'opacity-50 border-[#fbcfe8] cursor-not-allowed'
                      : 'border-[#fbcfe8] hover:border-[#f43f7e] hover:shadow-sm cursor-pointer active:scale-98'
                  }`}
                >
                  <div>
                    <div className="relative aspect-4/3 rounded-lg overflow-hidden bg-[#fff0f5] mb-2">
                      <img
                        src={prod.imageUrl}
                        alt={prod.name}
                        loading="lazy"
                      ...
                      />
                      <span className="absolute bottom-1 right-1 bg-[#380c25]/80 text-white text-[9px] px-1.5 py-0.5 rounded font-mono-craft">
                        {prod.sku}
                      </span>
                    </div>
                  ...
                  <div className="pt-2 mt-2 border-t border-[#fbcfe8] flex items-center justify-between">
                    <div>
                      <div className="text-xs font-bold text-[#380c25] font-mono-craft">
                    ...
                    <button
                      ...
                      className="w-7 h-7 rounded-lg bg-[#fff0f5] hover:bg-[#f43f7e] hover:text-white flex items-center justify-center text-[#db2777] transition-colors border border-[#fbcfe8]"
                    >
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-[#ffffff] rounded-2xl border border-[#fbcfe8] shadow-2xs p-5 flex flex-col justify-between">
            <div>
              {/* Cart Header */}
              <div className="flex items-center justify-between pb-3 border-b border-[#fbcfe8]">
                ...
                {cart.length > 0 && (
                  <button
                    ...
                    className="text-xs text-[#f43f7e] hover:text-[#db2777] hover:underline cursor-pointer font-mono-craft"
                  >
                    Limpar
                  </button>
                )}
              ...
              {/* Items List */}
              <div className="divide-y divide-[#fff0f5] max-h-60 overflow-y-auto my-3 pr-1">
                {cart.length === 0 ? (
                      ...
                      <div key={item.product.id} className="py-2.5 flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          <button
                            ...
                            className="w-6 h-6 rounded bg-[#fff0f5] hover:bg-[#fbcfe8] flex items-center justify-center text-[#380c25] cursor-pointer"
                          >
                            <Minus className="w-3 h-3" />
                          </button>
                          <span className="w-6 text-center text-xs font-bold text-[#380c25] font-mono-craft">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => handleUpdateQuantity(item.product.id, 1)}
                          ...
                          <button
                            ...
                            className="w-6 h-6 rounded text-[#9b4f76] hover:text-[#f43f7e] flex items-center justify-center ml-1 cursor-pointer"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </div>

                        <div className="text-right font-bold text-xs text-[#380c25] shrink-0 w-16 font-mono-craft">
              ...
              {brandBreakdown.length > 1 && (
                <div className="bg-[#fff0f5]/70 rounded-xl p-2.5 border border-[#fbcfe8] mb-3 text-[11px] text-[#380c25] space-y-1 font-mono-craft">
                  <div className="font-bold flex items-center gap-1 text-[#f43f7e]">
                    <Sparkles className="w-3 h-3 text-[#f43f7e]" />
                    Venda Compartilhada ({brandBreakdown.length} marcas):
                  </div>
                  {brandBreakdown.map((b) => (
                    <div key={b.brandName} className="flex justify-between text-[#863b63]">
                      <span>{b.brandName} ({b.count} itens):</span>
                      <span className="font-semibold text-[#380c25]">R$ {b.gross.toFixed(2).replace('.', ',')}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="space-y-3 pt-2 border-t border-[#fbcfe8]">
                <label className="block text-xs font-bold text-[#380c25] uppercase tracking-wide font-mono-craft">
                  Forma de Pagamento
                </label>
                  ...
                  <button
                    ...
                    className={`py-2 px-1 rounded-lg border text-center transition-all cursor-pointer ${
                      paymentMethod === 'CREDITO'
                        ? 'bg-[#380c25] text-white border-[#380c25] shadow-xs'
                        : 'bg-[#fff0f5] border-[#fbcfe8] text-[#380c25] hover:bg-[#fce7f3]'
                    }`}
                  >
                    Crédito
                  </button>
                ...
                {paymentMethod === 'CREDITO' && (
                  <div className="bg-[#fff0f5]/60 p-3 rounded-xl border border-[#fbcfe8] space-y-2.5 text-xs font-mono-craft">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-[11px] font-semibold text-[#863b63] mb-1">
                          Bandeira
                        </label>
                        <select
                          value={cardBrand}
                          onChange={(e) => setCardBrand(e.target.value as any)}
                          className="w-full p-2 bg-white rounded-lg border border-[#fbcfe8] text-xs focus:outline-none"
                        >
                ...
                {/* Debit Configuration */}
                {paymentMethod === 'DEBITO' && (
                  <div className="bg-[#fff0f5]/60 p-3 rounded-xl border border-[#fbcfe8] space-y-2 text-xs font-mono-craft">
                    <div className="flex items-center justify-between">
                      <span className="text-[#863b63]">Bandeira:</span>
                        ...
                        {(['VISA', 'MASTERCARD', 'ELO'] as const).map((b) => (
                          <button
                            ...
                            className={`px-2 py-1 rounded text-xs font-semibold cursor-pointer ${
                              cardBrand === b ? 'bg-[#380c25] text-white' : 'bg-white border border-[#fbcfe8] text-[#380c25]'
                            }`}
                          >
                            {b}
                          </button>
                    ...
                    <div className="text-[11px] text-[#863b63] flex justify-between">
                      <span>Taxa Stone Débito:</span>
                      <span className="font-semibold text-[#380c25]">{feePct}%</span>
                    </div>
                  </div>
                )}
                ...
                {paymentMethod === 'DINHEIRO' && (
                  <div className="bg-[#fff0f5]/60 p-3 rounded-xl border border-[#fbcfe8] space-y-2 text-xs font-mono-craft">
                    <div className="flex items-center justify-between gap-3">
                      <label className="text-[#863b63] font-semibold">Valor Recebido (R$):</label>
                      <input
                        type="number"
                        step="0.01"
                        value={cashReceived || ''}
                        ...
                        className="w-28 p-1.5 bg-white border border-[#fbcfe8] rounded text-right font-bold text-[#380c25]"
                      />
                    </div>
                ...
                <div className="pt-2 border-t border-[#fbcfe8] space-y-2">
                  <div className="flex items-center justify-between text-[11px] font-mono-craft">
                    <span className="font-semibold text-[#380c25] flex items-center gap-1.5">
                      <span className="text-emerald-700">📱</span> Comprovante Digital via WhatsApp
                    </span>
                    <span className="text-[10px] text-[#9b4f76] bg-[#fff0f5] px-2 py-0.5 rounded-full font-medium">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                    <div className="relative">
                      <input
                        ...
                        className="w-full p-2 bg-white border border-[#fbcfe8] rounded-xl text-xs text-[#380c25] focus:outline-none focus:border-[#1f4e38]"
                      />
                    </div>
                    <div className="relative">
                      <input
                        ...
                        className="w-full pl-7 pr-2 py-2 bg-white border border-[#fbcfe8] rounded-xl text-xs text-[#380c25] focus:outline-none focus:border-[#1f4e38]"
                      />
                      <span className="absolute left-2.5 top-2.5 text-[#9b4f76] text-[11px]">📱</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="pt-4 border-t border-[#fbcfe8] space-y-3 mt-4">
              <div className="bg-[#fff0f5]/50 rounded-xl p-3 text-xs space-y-1 border border-[#fbcfe8] font-mono-craft">
                <div className="flex justify-between text-[#863b63]">
                  <span>Subtotal Bruto:</span>
                  <span className="font-semibold text-[#380c25]">R$ {totalGross.toFixed(2).replace('.', ',')}</span>
                </div>
                <div className="flex justify-between text-[#863b63]">
                  <span>Taxa Máquina ({feePct}% congelada):</span>
                  <span className="text-[#f43f7e] font-medium">- R$ {estimatedFeeAmount.toFixed(2).replace('.', ',')}</span>
                </div>
                <div className="flex justify-between text-[#863b63]">
                  <span>Comissão Pinta e Borda (10%):</span>
                  <span className="text-[#863b63] font-medium">- R$ {estimatedCommissionAmount.toFixed(2).replace('.', ',')}</span>
                </div>
                <div className="flex justify-between text-[#1f4e38] font-bold pt-1 border-t border-[#fbcfe8]">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[11px] text-[#9b4f76] uppercase font-semibold font-mono-craft">Total a Cobrar</span>
                  <div className="text-2xl font-bold text-[#380c25] font-mono-craft">
