  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        >
          <PlusCircle className="w-4 h-4 text-[#ff7597]" />
      ...
      {lowStockProducts.length > 0 && (
        <div className="bg-[#fff0f5]/70 border border-[#fbcfe8] rounded-2xl p-4 flex items-start gap-3">
          <AlertTriangle className="w-5 h-5 text-[#f43f7e] shrink-0 mt-0.5" />
          <div className="text-xs">
            <h4 className="font-bold text-[#380c25] text-sm font-mono-craft">
              Alerta de Reposição: {lowStockProducts.length} itens com estoque mínimo atingido
            </h4>
            <p className="text-[#863b63] mt-0.5 font-light">
      ...
      <div className="flex items-center gap-2 border-b border-[#fbcfe8] font-mono-craft">
        <button
          onClick={() => setActiveTab('BALANCES')}
          className={`pb-3 px-3 text-xs font-semibold transition-colors border-b-2 cursor-pointer ${
            activeTab === 'BALANCES'
              ? 'border-[#380c25] text-[#380c25]'
      ...
      {/* Filter Row */}
      <div className="bg-[#ffffff] rounded-2xl p-4 border border-[#fbcfe8] shadow-2xs flex flex-col sm:flex-row gap-3 items-center justify-between">
        ...
        {userRole === 'ADMIN' && (
          <div className="w-full sm:w-64 font-mono-craft">
            <select
              ...
              className="w-full p-2 text-xs bg-white text-[#380c25] rounded-xl border border-[#fbcfe8] focus:outline-none"
            >
                  ...
                  return (
                    <tr key={prod.id} className="hover:bg-[#fff0f5]/40 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <span className="font-medium text-[#380c25]">{partner?.brandName}</span>
                      </td>
                      <td className="py-3 px-4 text-right font-bold font-mono-craft text-[#380c25]">
                        R$ {prod.price.toFixed(2).replace('.', ',')}
                      </td>
                      <td className="py-3 px-4 text-center font-bold font-mono-craft text-sm text-[#380c25]">
                        {prod.stock} un.
                      </td>
                      <td className="py-3 px-4 text-center font-mono-craft text-[#9b4f76]">
                        {prod.minStock} un.
                      </td>
                      <td className="py-3 px-4 font-mono-craft">
                        {isOut ? (
                          ...
                          <span className="px-2 py-0.5 rounded-full bg-[#ffe4ee] text-[#f43f7e] border border-[#fce7f3] text-[10px] font-bold">
                            Estoque Baixo
                          </span>
                        ) : (
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-[#fff0f5] text-[#380c25]">
                {visibleMovements.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="py-8 text-center text-[#9b4f76] font-mono-craft">
                  ...
                  visibleMovements.map((mov) => {
                    ...
                    return (
                      <tr key={mov.id} className="hover:bg-[#fff0f5]/40 transition-colors">
                        <td className="py-3 px-4 text-[#9b4f76] font-mono-craft whitespace-nowrap">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                              mov.type === 'ENTRADA'
                                ...
                                : 'bg-[#fff0f5] text-[#380c25] border border-[#fbcfe8]'
                            }`}
                          >
                            {mov.type}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center font-mono-craft text-[#9b4f76]">{mov.quantityBefore}</td>
                        <td className="py-3 px-4 text-center font-bold font-mono-craft">
                          <span className={isPositive ? 'text-[#3c6b54]' : 'text-[#f43f7e]'}>
                            {isPositive ? `+${mov.quantityChanged}` : mov.quantityChanged}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center font-bold font-mono-craft text-[#380c25]">
                          {mov.quantityAfter}
                        </td>
                        <td className="py-3 px-4 text-[#863b63] max-w-xs truncate font-light">
                          {mov.reason}
                        </td>
                        <td className="py-3 px-4 text-[#9b4f76] font-mono-craft">{mov.operatorName}</td>
                      </tr>
                    );
                  })
                )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#380c25]/60 backdrop-blur-xs p-4">
          <div className="bg-[#ffffff] rounded-3xl max-w-md w-full shadow-2xl border border-[#fbcfe8] p-6 space-y-4 animate-in fade-in zoom-in-95">
            ...
            <form onSubmit={handleCreateMovementSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-[#380c25] mb-1 font-mono-craft">Produto</label>
                <select
                  value={modalProductId}
                  onChange={(e) => setModalProductId(e.target.value)}
                  className="w-full p-2.5 bg-white rounded-xl border border-[#fbcfe8] text-xs text-[#380c25] focus:outline-none"
                >
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block font-semibold text-[#380c25] mb-1 font-mono-craft">Tipo de Movimento</label>
                  <select
                    value={modalType}
                ...
                <div>
                  <label className="block font-semibold text-[#380c25] mb-1 font-mono-craft">Quantidade</label>
                  <input
                    type="number"
                    min={1}
                    value={modalQuantity}
              ...
              <div>
                <label className="block font-semibold text-[#380c25] mb-1 font-mono-craft">Motivo do Lançamento</label>
                <input
                  type="text"
                  value={modalReason}
