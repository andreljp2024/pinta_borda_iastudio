...
export const VirtualStoreView: React.FC<{ onOpenAuth: () => void }> = ({ onOpenAuth }) => {
  ...
  return (
    <div className="min-h-screen bg-[#fff5f8] pb-24 text-[#380c25]">
      {/* Header Banner */}
      <div className="bg-[#380c25] text-[#ffffff] border-b border-[#5c1a3e]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-3 max-w-2xl">
              <button
                onClick={() => setActiveView('landing')}
                className="inline-flex items-center gap-1.5 text-xs font-mono-craft text-[#ff7597] hover:text-[#ffffff] transition-colors cursor-pointer mb-1"
              >
            ...
            <div className="bg-[#2f0a1e] rounded-2xl p-4 border border-[#5c1a3e] text-xs space-y-2 shrink-0 md:max-w-xs shadow-md">
              <div className="flex items-center gap-2 text-[#ff7597] font-medium font-mono-craft">
      ...
      <div className="sticky top-16 z-20 bg-[#ffffff]/95 backdrop-blur-md border-b border-[#fbcfe8] py-4 shadow-2xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            {/* Search Input */}
            <div className="relative flex-1">
              <Search className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-[#9b4f76]" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Buscar por peça, técnica, SKU ou ateliê..."
                className="w-full pl-10 pr-4 py-2.5 rounded-full border border-[#fbcfe8] bg-[#fff5f8] text-xs font-mono-craft text-[#380c25] placeholder-[#9b4f76] focus:outline-hidden focus:border-[#f43f7e] transition-all"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute right-3.5 top-1/2 -translate-y-1/2 text-xs font-mono-craft text-[#9b4f76] hover:text-[#380c25] cursor-pointer"
                >
                  Limpar
                </button>
              )}
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                className="px-3.5 py-2.5 rounded-full border border-[#fbcfe8] bg-[#fff5f8] text-xs font-mono-craft text-[#380c25] focus:outline-hidden focus:border-[#f43f7e] cursor-pointer"
              >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full font-mono-craft whitespace-nowrap transition-all cursor-pointer text-xs ${
                selectedCategory === 'ALL'
                  ? 'bg-[#380c25] text-[#ffffff] font-medium shadow-xs'
                  : 'bg-[#fff0f5] text-[#863b63] hover:bg-[#fbcfe8] hover:text-[#380c25]'
              }`}
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        {currentPartnerInfo && (
          <div className="mb-8 bg-[#ffffff] rounded-3xl p-6 border border-[#fbcfe8] shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <img
                ...
                className="w-16 h-16 rounded-2xl object-cover border border-[#fbcfe8] shadow-2xs"
              />
              <div>
                <div className="flex items-center gap-2">
                  <span className="px-2.5 py-0.5 rounded-full bg-[#fff0f5] text-[#db2777] font-mono-craft text-[10px] tracking-wider uppercase border border-[#fbcfe8]">
                    Ateliê Autoral Maranhense
                  </span>
                </div>
                <h2 className="font-display text-2xl font-medium text-[#380c25] mt-0.5">
                  {currentPartnerInfo.brandName}
                </h2>
                <p className="text-xs text-[#863b63] mt-0.5 font-light">
                  Artesã criadora: <strong className="text-[#380c25] font-medium">{currentPartnerInfo.ownerName}</strong>
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <button
                ...
                className="px-3.5 py-2 rounded-full bg-[#fff0f5] hover:bg-[#fbcfe8] text-[#380c25] font-mono-craft text-xs transition-colors cursor-pointer border border-[#fbcfe8]"
              >
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="text-xs text-[#863b63] font-mono-craft">
            Exibindo <strong className="text-[#380c25]">{filteredProducts.length}</strong> peças autorais
            {selectedPartner !== 'ALL' && (
              <span className="ml-1 text-[#f43f7e]">
        ...
        {filteredProducts.length === 0 ? (
          <div className="bg-[#ffffff] rounded-3xl p-12 text-center border border-[#fbcfe8] space-y-4 max-w-md mx-auto my-12 shadow-xs">
            <div className="w-12 h-12 rounded-full bg-[#fff0f5] text-[#db2777] flex items-center justify-center mx-auto">
              <ShoppingBag className="w-6 h-6" />
            </div>
            <h3 className="font-display text-xl font-medium text-[#380c25]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="bg-[#ffffff] rounded-3xl border border-[#fbcfe8] overflow-hidden hover:border-[#f43f7e] hover:shadow-lg transition-all flex flex-col group p-3.5"
                >
                  {/* Product Image */}
                  <div
                    onClick={() => setSelectedProduct(product)}
                    className="relative aspect-square rounded-2xl overflow-hidden bg-[#fff0f5] cursor-pointer"
                  >
                    <img
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-1.5 items-start">
                      <span className="px-2.5 py-1 rounded-full bg-[#380c25]/85 backdrop-blur-xs text-[#ffffff] text-[10px] font-mono-craft">
                        {product.partnerName}
                      </span>
                      {product.isFeatured && (
                        <span className="px-2 py-0.5 rounded-full bg-[#f43f7e] text-white text-[9px] font-mono-craft flex items-center gap-1 shadow-2xs">
                          <Sparkles className="w-2.5 h-2.5 text-[#ff7597]" /> Destaque
                        </span>
                      )}
                    </div>
                    ...
                    <div className="absolute bottom-3 right-3">
                      {isOutOfStock ? (
                        <span className="px-2 py-1 rounded-md bg-[#380c25]/85 text-[#c9d9d0] text-[10px] font-mono-craft">
                          Sob encomenda
                        </span>
                      ) : isLowStock ? (
                        <span className="px-2 py-1 rounded-md bg-[#fff0f5] text-[#db2777] text-[10px] font-mono-craft font-bold border border-[#fbcfe8]">
                          Últimas {product.stockPhysical} un.
                        </span>
                      ) : (
                        <span className="px-2 py-1 rounded-md bg-[#ffffff]/95 text-[#380c25] text-[10px] font-mono-craft border border-[#fbcfe8] shadow-2xs">
                  ...
                  <div className="pt-3 px-1 flex-1 flex flex-col justify-between space-y-3">
                    <div>
                      <div className="flex items-center justify-between text-[10px] text-[#9b4f76] font-mono-craft mb-1">
                        <span>{product.sku}</span>
                        <span>{product.category}</span>
                      </div>
                      <h3
                        onClick={() => setSelectedProduct(product)}
                        className="font-display font-medium text-[#380c25] text-lg leading-snug group-hover:text-[#f43f7e] transition-colors cursor-pointer line-clamp-2"
                      >
                        {product.name}
                      </h3>
                      <p className="text-xs text-[#863b63] mt-1 line-clamp-2 leading-relaxed font-light">
                        {product.description}
                      </p>
                    </div>

                    <div className="pt-2 border-t border-[#fbcfe8]/70 space-y-3">
                      <div className="flex items-baseline justify-between">
                        <div>
                          <span className="text-[10px] text-[#9b4f76] block uppercase font-mono-craft">
                            Preço no Shopping
                          </span>
                        ...
                        {product.price >= 60 && (
                          <span className="text-[10px] text-[#9b4f76] font-mono-craft">
                            em até 3x no cartão
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => setSelectedProduct(product)}
                          className="flex-1 py-2 px-3 bg-[#fff0f5] hover:bg-[#fbcfe8] text-[#380c25] rounded-xl text-xs font-mono-craft transition-colors cursor-pointer border border-[#fbcfe8]"
                        >
                          Ver Detalhes
                        </button>
                        ...
                        <a
                          ...
                          className="p-2.5 bg-[#f43f7e] hover:bg-[#f43f7e] text-white rounded-xl transition-colors cursor-pointer shrink-0 shadow-2xs"
                        >
                          <MessageCircle className="w-4 h-4" />
                        </a>
                      </div>
